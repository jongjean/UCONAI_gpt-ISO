            ...
            {editing ? (
              <>
                ...
                <textarea
                  ...
                  placeholder="지침 내용 또는 ISO 작성 가이드라인을 입력하세요."
                  style={{
                    flex: 1,
                    width: "100%",
                    borderRadius: 6,
                ...
                >
                  ※ 파일 첨부/드래그앤드롭, 서버 저장 연동은 다음 버전에서 확장할 수
                  있습니다.
                </div>
              </>
            ) : (
      ...
      const assistantMessage: Message = {
        ...
        content:
          assistantText && assistantText.trim().length > 0
            ? assistantText
            : "정확한 정보가 부족하지만, 향후 Deep Learning을 위해 관련 자료(초안, TR, 회의 문서 등)를 제공해주시면 더 구체적으로 도와드릴 수 있습니다.",
      };
      ...
      <aside className="iso-sidebar">
        <div className="iso-sidebar-header">
          ...
          <div className="iso-sidebar-sub">
            ISO/IEC JTC 1 SC 36 · PWI 26255 · TR 25468 국제 표준 작업 전용
            어시스턴트
          </div>
          <button
                ...
                <div style={{position:'relative', minHeight: 16}}>
                  {conversations.length > 1 && (
                    <button
                      ...
                      tabIndex={-1}
                    >
                      <span style={{fontSize: 10, lineHeight: 1, pointerEvents: 'none'}}>×</span>
                    </button>
                  )}
                  <div
                  ...
                  <div className="iso-sidebar-conv-meta">
                    {(() => {
                      // Extract date (YYYY. MM. DD.) and 오전/오후 HH:MM from c.createdAt
                      // Example: '2025. 11. 22. 오후 6:56:10'
                      const dateMatch = c.createdAt.match(/^(\d{4}\. ?\d{1,2}\. ?\d{1,2}\.)/);
                      const timeMatch = c.createdAt.match(/(오전|오후)\s*\d{1,2}:\d{2}/);
                      const date = dateMatch ? dateMatch[1].trim() : '';
                      const time = timeMatch ? timeMatch[0] : '';
                      return (date + (time ? ' ' + time : '')) + ' · ' + c.messages.length + ' 메시지';
                    })()}
                  </div>
                </div>
              </li>
      ...
      <main className="iso-main">
        <div className="iso-main-card">
            ...
            <div className="iso-main-control" style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 32, justifyContent: "center", marginTop: 4 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ fontSize: 13, color: answerMode === "strict" ? "#5b21b6" : "#6b7280", marginTop: 2, fontWeight: 700 }}>실체적 정보만 제공</div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <button
                    type="button"
                  ...
                  <div style={{ fontSize: 13, color: answerMode === "aggressive" ? "#5b21b6" : "#6b7280", marginTop: 2, fontWeight: 700 }}>생성형AI 정보 포함</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="iso-main-description">
            ISO/IEC 표준, PWI 26255, TR 25468, 메타버스 LET 등에 대해 질문하면, 보수형 또는 적극형 모드에 따라 답변합니다.
          </div>
